-- Incremento 4.5 — lifecycle minimo de Room.
--
-- RoomParticipant sigue representando membresia. Un no-host que sale elimina
-- su membresia; el slot activo se libera por la FK ON DELETE CASCADE existente.
-- Cerrar una Room preserva las membresias historicas y libera todos los slots
-- por el trigger rooms_release_active_slots() existente.
...
begin
  ...
  if current_auth_user_id is null then
    raise exception 'Se necesita una AuthIdentity valida para salir de una sala.'
      using errcode = '28000';
  end if;
...
begin
  ...
  if current_auth_user_id is null then
    raise exception 'Se necesita una AuthIdentity valida para cerrar una sala.'
      using errcode = '28000';
  end if;
  ...
  if active_room_id is null or active_room_status <> 'lobby' then
    raise exception 'No tenes una sala activa para cerrar.'
      using errcode = 'P0015';
  end if;

  if active_room_host_player_id <> current_player_id then
    raise exception 'Solo el host puede cerrar la sala.'
      using errcode = 'P0016';
  end if;
...
revoke all on function public.close_room() from public;
grant execute on function public.close_room() to authenticated;

create or replace function public.is_current_player_room_lifecycle_observer(target_room_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function public.is_current_player_room_lifecycle_observer(uuid) from public;
grant execute on function public.is_current_player_room_lifecycle_observer(uuid) to authenticated;

drop policy "Room participants can read their lobby room" on public.rooms;

create policy "Room participants can read their lifecycle room"
  on public.rooms
  for select
  to authenticated
  using (public.is_current_player_room_lifecycle_observer(id));
