...
function nicknames(rows) {
  return rows.map((row) => row.participant_nickname).sort();
}
...
async function subscribeWithAuthoritativeRefetch(client, label, roomId, snapshots) {
  ...
  const channel = client
    .channel(`smoke-4-4:${label}:${roomId}`)
    .on("system", {}, (payload) => {
      if (
...
async function validate() {
  ...
  const createdA = await createGroup(clientA, "Familia smoke 4.4", "Ramiro");
  const roomRowsA = await createRoom(clientA);
  const roomCode = roomRowsA[0].room_join_code;
  const roomId = roomRowsA[0].room_id;

  assert(roomId, "create_room must return room_id for Realtime filter.");
  ...
  assert(subscriptionA.eventCount >= 1, "A must receive at least one B join event.");
  assertEqual(subscriptionA.refetchCount, subscriptionA.eventCount, "A must refetch from RPC for every observed event.");
  results.push(["A lobby abierto / B join", "PASS", `A refetched ${nicknames(snapshotsA.at(-1)).join(",")}.`]);
  ...
  const clientC = await signInAnonymously("C");
  await joinGroup(clientC, createdA.invitation_code, "Julia");
  await joinRoomByCode(clientC, roomCode);

  await waitUntil(
    () => snapshotsA.some((rows) => includesNicknames(rows, ["Ramiro", "Pedro", "Julia"])),
    "A to refetch A+B+C"
  );
  await waitUntil(
    () => snapshotsB.some((rows) => includesNicknames(rows, ["Ramiro", "Pedro", "Julia"])),
    "B to refetch A+B+C"
  );
  results.push(["C join", "PASS", "A y B convergen a Ramiro, Pedro, Julia sin refresh manual."]);

  const clientOther = await signInAnonymously("Other");
  await createGroup(clientOther, "Familia smoke 4.4 ajena", "Ajena");
  ...
  const clientD = await signInAnonymously("D");
  await joinGroup(clientD, createdA.invitation_code, "Laura");
  await joinRoomByCode(clientD, roomCode);

  await waitUntil(
    () => snapshotsA.some((rows) => includesNicknames(rows, ["Ramiro", "Pedro", "Julia", "Laura"])),
    "A to refetch A+B+C+D"
  );
  await new Promise((resolve) => setTimeout(resolve, 1000));

  assertEqual(foreignSubscription.eventCount, 0, "Foreign Group subscription must not receive Room A events.");
  assertEqual(foreignSnapshots.length, 0, "Foreign Group subscription must not refetch Room A lobby.");
  results.push(["aislamiento otro Group", "PASS", "Filtro manipulado hacia Room A no entregó eventos útiles al otro Group."]);
  ...
  results.push(["cleanup", "PASS", "removeChannel dejó los clientes sin channels activos."]);

  const refreshedA = await getMyActiveRoomRows(clientA);
  const refreshedB = await getMyActiveRoomRows(clientB);

  assert(includesNicknames(refreshedA, ["Ramiro", "Pedro", "Julia", "Laura"]), "A refresh reconstruction mismatch.");
  assert(includesNicknames(refreshedB, ["Ramiro", "Pedro", "Julia", "Laura"]), "B refresh reconstruction mismatch.");
  results.push(["refresh 4.3", "PASS", "A y B reconstruyen lobby por get_my_active_room()."]);
